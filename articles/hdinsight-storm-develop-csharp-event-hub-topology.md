...
4. Find the following code:
	...
	This creates a spout and uses the Event Hub partition count as the parallelism hint for this component. This should create an instance of the spout for each partition. This also declares that the output stream should use the deserializer created previously.
...
1. In **Solution Explorer**, right-click the **EventHubReader** project, and select **Manage Nuget Packages**. Search for the **Json.NET** package, then add that to the solution. This allows you to easily process the JSON data received from the spout. Also add the **Windows Azure Storage** package, which allows you to write to Table storage.
...
4. Find the following code:
	...
	This instructs the bolt that it will be receiving a **string** value instead of an **int** value, and that the data should be deserialized by using the **CustomizedInteropJSONDeserialzer**.
...
In this tutorial, you have learned how to use the Java Event Hub spout and bolt from a C# topology to work with data in Azure Event Hub. To learn more about creating C# topologies, see the following:
